...
function($, template, Backbone, tpl,model,attachment,platformModel){
    ...
    return Backbone.View.extend({
        ...
        events:{
            ...
            'click #js_uploadImgBtn' : 'uploadScreenShot',  /* 上传截图 */
        ...
        render: function(data) {
            ...
            /* 初始化应用下载 */
            if (this.model.get('appinfos')) {
                if (this.model.get('appinfos').length > 0) {
                    _.each(this.model.get('appinfos'),function(value,key,list){
                        $('#js_appinfos tr').eq(key).find('select[appinfoAttr=js_platform]').val(value.platform);
                    });
                };
            }else{
                this.model.set('appinfos',[]);
            }
        ...
        submit:function(e){
            ...
            var that = this;
            submitObject.appinfos = [];
            $('#js_appinfos tr').each(function(index,e){
                submitObject.appinfos.push({
                    id: $(this).attr('appinfoId'),
                    application_id: that.model.get('id'),
                ...
                if (!submitObject.appinfos[index].id) {
                    delete submitObject.appinfos[index].id;
                };
            ...
            this.model.update(submitObject,function(data){
                if (data.errCode == 0) {
                    if ($(e.target).attr('skip')) {
                        ...
                        var html = [
                            '<p>3秒后或任意点击，返回编辑页面</p>',
                            '<a href="#application/list">跳转到应用列表</a>'
                        ].join('');
