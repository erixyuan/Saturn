define([
    ...
    'template',
    'backbone',
    'text!../../template/paginationPlugin.html',
    ],
...
function($, template, Backbone, paginationTpl){
    ...
    return Backbone.View.extend({
        ...
        render: function(obj) {
            ...
            var options = {
                id : obj.id,      //载入分页器的id
                data : obj.data,    //分页需要的数据
                callback :obj.callback,
                parent : obj.parent
            }

            var context = options.data;     // 分页数据,包括最后一页，当前页，当前状态状态
            if ($('#js_pagePluginTpl').length == 0) {
            ...
            var html = template.render('js_pagePluginTpl',numberBoxs);
            $(options.id).html(html);

            /* 绑定按钮事件 */
            $(options.id).find('ul[js_numbers] li a, a[next], a[prev]').click(function(e){
                options.parent.page = $(e.target).attr('page');
                options.callback();
            });
            // 绑定跳转按钮事件
            $(options.id).find('a[js_jumpPageBtn]').click(function(){
                var value = $(options.id).find('input[js_jumpPageValue]').val();
                if (value && parseInt(value) != NaN) {
                    options.parent.page = value;
                    options.callback();
                };
            })
