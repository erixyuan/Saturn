define([
    ...
    'template',
    'backbone',
    'text!../../../template/product/list.html',
    '../../model/product/list',
    '../PaginationView',
    ],
...
function($, template, Backbone, tpl,model,PaginationView){
    ...
    return Backbone.View.extend({
        ...
        initialize: function(obj){
            ...
            this.model = new this.model(status,page,obj.keyword,obj.category,obj.author);  // 3.初始化模型
            this.model.fetch({
        ...
        render: function(data) {
            ...
            if(!Saturn.isCurrentView("product","list")){
                return false;
            }
            ...
            var paginationSetting = {
                                    url : this.model.get('keyword') ?
                                               '#product/list/'+this.status+'/'+this.model.get('keyword') :
                                               '#product/list/'+this.status,
                                    params: {},
        ...
        batchOperate:function(){
            ...
            $('#js_productListContent input[type=checkbox][operateId]:checked').each(function(){
        ...
        search:function(){
            ...
            // 组装hash请求
            window.location.hash = '#product/list/'+this.status+'/'+keyword+"/1";
        },
